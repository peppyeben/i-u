...
(function(e1) {
    ...
    var Webcam = {
        ...
        live: false,
        userMedia: true,
        iOS: /iPad|iPhone|iPod/.test(navigator.userAgent) && !e1.MSStream,
        params: {
            width: 0,
            height: 0,
            dest_width: 0,
        ...
        init: function() {
            ...
            e1.URL = e1.URL || e1.webkitURL || e1.mozURL || e1.msURL;
            this.userMedia = this.userMedia && !!this.mediaDevices && !!e1.URL;
            if (navigator.userAgent.match(/Firefox\D+(\d+)/)) {
                if (parseInt(RegExp.$1, 10) < 21) this.userMedia = null;
            }
            if (this.userMedia) e1.addEventListener("beforeunload", function(e) {
                t2.reset();
            });
        },
        exifOrientation: function(e) {
            ...
            while(a < e.byteLength){
                ...
                if (i == 225) {
                    ...
                    a += 6;
                    var r = null;
                    if (t.getUint16(a) == 18761) r = false;
                    else if (t.getUint16(a) == 19789) r = true;
                    else {
        ...
        attach: function(a2) {
            if (typeof a2 == "string") a2 = document.getElementById(a2) || document.querySelector(a2);
            if (!a2) return this.dispatch("error", new i1("Could not locate DOM element to attach to."));
            this.container = a2;
            a2.innerHTML = "";
            var s2 = document.createElement("div");
            a2.appendChild(s2);
            this.peg = s2;
            if (!this.params.width) this.params.width = a2.offsetWidth;
            if (!this.params.height) this.params.height = a2.offsetHeight;
            if (!this.params.width || !this.params.height) return this.dispatch("error", new i1("No width and/or height for webcam.  Please call set() first, or attach to a visible element."));
            if (!this.params.dest_width) this.params.dest_width = this.params.width;
            if (!this.params.dest_height) this.params.dest_height = this.params.height;
            this.userMedia = t1 === undefined ? this.userMedia : t1;
            if (this.params.force_flash) {
                t1 = this.userMedia;
                this.userMedia = null;
            }
            if (typeof this.params.fps !== "number") this.params.fps = 30;
            var r1 = this.params.width / this.params.dest_width;
            var o1 = this.params.height / this.params.dest_height;
            if (this.userMedia) {
                ...
                if (r1 != 1 || o1 != 1) {
                    ...
                    n.style.webkitTransform = "scaleX(" + r1 + ") scaleY(" + o1 + ")";
                    n.style.mozTransform = "scaleX(" + r1 + ") scaleY(" + o1 + ")";
                    n.style.msTransform = "scaleX(" + r1 + ") scaleY(" + o1 + ")";
                    n.style.oTransform = "scaleX(" + r1 + ") scaleY(" + o1 + ")";
                    n.style.transform = "scaleX(" + r1 + ") scaleY(" + o1 + ")";
                }
                a2.appendChild(n);
                this.video = n;
                var l1 = this;
                this.mediaDevices.getUserMedia({
                    ...
                    if ("srcObject" in n) n.srcObject = t;
                    else n.src = e1.URL.createObjectURL(t);
                }).catch(function(e) {
                    if (l1.params.enable_flash && l1.detectFlash()) setTimeout(function() {
                        l1.params.force_flash = 1;
                        l1.attach(a2);
                    }, 1);
                    else l1.dispatch("error", e);
                });
            } else if (this.iOS) {
                ...
                var l1 = this;
                var m = this.params;
                f.addEventListener("change", function(e2) {
                    if (e2.target.files.length > 0 && e2.target.files[0].type.indexOf("image/") == 0) {
                        var t3 = URL.createObjectURL(e2.target.files[0]);
                        var a = new Image;
                        ...
                        i2.addEventListener("load", function(e) {
                            var i = l1.exifOrientation(e.target.result);
                            if (i > 1) l1.fixOrientation(t3, i, a);
                            else a.src = t3;
                        }, false);
                        var s3 = new XMLHttpRequest;
                        s3.open("GET", t3, true);
                        s3.responseType = "blob";
                        s3.onload = function(e) {
                            if (this.status == 200 || this.status === 0) i2.readAsArrayBuffer(this.response);
                        };
                        s3.send();
                    }
                }, false);
                ...
                h.addEventListener("click", function(e) {
                    if (m.user_callback) l1.snap(m.user_callback, m.user_canvas);
                    else {
                ...
                a2.appendChild(h);
            } else this.dispatch("error", new i1(this.params.noInterfaceFoundText));
            if (this.params.crop_width && this.params.crop_height) {
                var p = Math.floor(this.params.crop_width * r1);
                var u = Math.floor(this.params.crop_height * o1);
                a2.style.width = "" + p + "px";
                a2.style.height = "" + u + "px";
                a2.style.overflow = "hidden";
                a2.scrollLeft = Math.floor(this.params.width / 2 - p / 2);
                a2.scrollTop = Math.floor(this.params.height / 2 - u / 2);
            } else {
                a2.style.width = "" + this.params.width + "px";
                a2.style.height = "" + this.params.height + "px";
            }
        },
        reset: function() {
        ...
        set: function() {
            if (arguments.length == 1) for(var e in arguments[0])this.params[e] = arguments[0][e];
        ...
        dispatch: function() {
            ...
            if (this.hooks[t] && this.hooks[t].length) {
                for(var r = 0, o = this.hooks[t].length; r < o; r++){
                    ...
                    if (typeof n == "function") n.apply(this, s);
                    else if (typeof n == "object" && n.length == 2) n[0][n[1]].apply(n[0], s);
                    else if (e1[n]) e1[n].apply(e1, s);
                }
                return true;
            } else if (t == "error") {
                var l;
                if (s[0] instanceof a1 || s[0] instanceof i1) l = s[0].message;
                else l = "Could not access webcam: " + s[0].name + ": " + s[0].message + " " + s[0].toString();
        ...
        detectFlash: function() {
            var t = "Shockwave Flash", a = "ShockwaveFlash.ShockwaveFlash", i = "application/x-shockwave-flash", s = e1, r = navigator, o = false;
            if (typeof r.plugins !== "undefined" && typeof r.plugins[t] === "object") {
                var n = r.plugins[t].description;
                if (n && typeof r.mimeTypes !== "undefined" && r.mimeTypes[i] && r.mimeTypes[i].enabledPlugin) o = true;
            } else if (typeof s.ActiveXObject !== "undefined") try {
        ...
        getSWFHTML: function() {
            ...
            if (location.protocol.match(/file/)) {
                this.dispatch("error", new a1("Flash does not work from local disk.  Please run from a web server."));
                return '<h3 style="color:red">ERROR: the Webcam.js Flash fallback does not work from local disk.  Please run it from a web server.</h3>';
            }
            if (!this.detectFlash()) {
                this.dispatch("error", new a1("Adobe Flash Player not found.  Please install from get.adobe.com/flashplayer and try again."));
            ...
            if (e1.localStorage && !localStorage.getItem("visited")) {
        ...
        getMovie: function() {
            if (!this.loaded) return this.dispatch("error", new a1("Flash Movie is not loaded yet"));
            var e = document.getElementById("webcam_movie_obj");
            if (!e || !e._snap) e = document.getElementById("webcam_movie_embed");
            if (!e) this.dispatch("error", new a1("Cannot locate Flash movie in DOM"));
            return e;
        },
        freeze: function() {
            var e = this;
        ...
        snap: function(e, t) {
            ...
            var a3 = this;
            var s = this.params;
            if (!this.loaded) return this.dispatch("error", new i1("Webcam is not loaded yet"));
            if (!e) return this.dispatch("error", new i1("Please provide a callback function or canvas to snap()"));
            if (this.preview_active) {
        ...
        base64DecToArr: function(e, t) {
            var a = e.replace(/[^A-Za-z0-9\+\/]/g, ""), i = a.length, s = t ? Math.ceil((i * 3 + 1 >> 2) / t) * t : i * 3 + 1 >> 2, r = new Uint8Array(s);
            for(var o, n, l = 0, h = 0, c = 0; c < i; c++){
                n = c & 3;
                l |= this.b64ToUint6(a.charCodeAt(c)) << 18 - 6 * n;
                if (n === 3 || i - c === 1) {
                    for(o = 0; o < 3 && h < s; o++, h++)r[h] = l >>> (16 >>> o & 24) & 255;
                    l = 0;
                }
            }
            return r;
        },
        upload: function(e3, t5, a) {
            var i = this.params.upload_name || "webcam";
            var s = "";
            if (e3.match(/^data\:image\/(\w+)/)) s = RegExp.$1;
            else throw "Cannot locate image format in Data URI";
            var r = e3.replace(/^data\:image\/\w+\;base64\,/, "");
    ...
    else e1.Webcam = Webcam;
})(window);

//# sourceMappingURL=index.7554449d.js.map
